...
public class CommandMailClear {
	...
	public CommandMailClear(CommandSource sender, String[] args) {
		...
		if(!PermissionsUtils.has(sender, "core.mail.clear")) { sender.sendMessage(Text.builder("You do not have permissions!").color(TextColors.RED).build()); return; }

		if(args.length != 1) { sender.sendMessage(Text.of(TextColors.YELLOW, "Usage: ", TextColors.GRAY, "/mail clear")); return; }
