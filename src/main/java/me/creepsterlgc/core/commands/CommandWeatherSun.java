...
public class CommandWeatherSun {
	...
	public CommandWeatherSun(CommandSource sender, String[] args, Game game) {
		...
		if(!PermissionsUtils.has(sender, "core.weather.sun")) { sender.sendMessage(Text.builder("You do not have permissions!").color(TextColors.RED).build()); return; }

		if(args.length == 2) {

			if(args[1].equalsIgnoreCase("*")) {
				for(World world : Controller.getServer().getWorlds()) world.setWeather(Weathers.CLEAR);
				ServerUtils.broadcast(Text.of(TextColors.YELLOW, sender.getName(), TextColors.GRAY, " has changed the weather to sun."));
				return;
			}
			...
			ServerUtils.broadcast(Text.of(TextColors.YELLOW, sender.getName(), TextColors.GRAY, " has changed the weather to sun on ", TextColors.YELLOW, world.getName()));
