...
public class CommandWarpInfo {
	...
	public CommandWarpInfo(CommandSource sender, String[] args) {
		...
		if(!PermissionsUtils.has(sender, "core.warp.info")) { sender.sendMessage(Texts.builder("You do not have permissions!").color(TextColors.RED).build()); return; }
		...
		sender.sendMessage(Texts.of(TextColors.GRAY, "Owner: ", TextColors.YELLOW, CoreDatabase.getPlayer(warp.getOwner()).getName()));
