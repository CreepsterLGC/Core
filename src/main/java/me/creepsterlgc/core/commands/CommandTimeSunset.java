...
public class CommandTimeSunset {
	...
	public CommandTimeSunset(CommandSource sender, String[] args, Game game) {
		...
		if(!PermissionsUtils.has(sender, "core.time.sunset")) { sender.sendMessage(Text.builder("You do not have permissions!").color(TextColors.RED).build()); return; }

		if(args.length == 2) {

			if(args[1].equalsIgnoreCase("*")) {
				for(World world : Controller.getServer().getWorlds()) world.getProperties().setWorldTime(12500);
				ServerUtils.broadcast(Text.of(TextColors.YELLOW, sender.getName(), TextColors.GRAY, " has changed the time to sunset."));
				return;
			}
			...
			ServerUtils.broadcast(Text.of(TextColors.YELLOW, sender.getName(), TextColors.GRAY, " has changed the time to sunset on ", TextColors.YELLOW, world.getName()));
