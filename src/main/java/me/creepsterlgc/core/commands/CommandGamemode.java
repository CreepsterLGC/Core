...
public class CommandGamemode implements CommandCallable {
	...
	public CommandResult process(CommandSource sender, String arguments) throws CommandException {
		...
		if(!PermissionsUtils.has(sender, "core.gamemode")) { sender.sendMessage(Text.builder("You do not have permissions!").color(TextColors.RED).build()); return CommandResult.success(); }

		if(arguments.equalsIgnoreCase("")) { sender.sendMessage(Text.of(TextColors.YELLOW, "Usage: ", TextColors.GRAY, "/gm <mode> [player]")); return CommandResult.success(); }

		if(args.length < 1 || args.length > 2) { sender.sendMessage(Text.of(TextColors.YELLOW, "Usage: ", TextColors.GRAY, "/gm <mode> [player]")); return CommandResult.success(); }
		...
		if(args.length == 1) {
			...
			sender.sendMessage(Text.of(TextColors.GRAY, "Your gamemode has been set to ", TextColors.YELLOW, gm.getName()));
		...
		if(args.length == 2) {
			...
			player.offer(Keys.GAME_MODE, gm);
			player.sendMessage(Text.of(TextColors.YELLOW, sender.getName(), Text.of(TextColors.GRAY, " has set your gamemode to ", TextColors.YELLOW, gm.getName())));
			sender.sendMessage(Text.of(TextColors.YELLOW, player.getName(), "'s", TextColors.GRAY, " gamemode has been set to ", TextColors.YELLOW, gm.getName()));
