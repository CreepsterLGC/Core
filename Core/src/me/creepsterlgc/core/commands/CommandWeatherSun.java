...
public class CommandWeatherSun {
	...
	public CommandWeatherSun(CommandSource sender, String[] args, Game game) {
		...
		if(!PERMISSIONS.has(sender, "core.weather.sun")) { sender.sendMessage(Texts.builder("You do not have permissions!").color(TextColors.RED).build()); return; }
		...
		Text t2 = Texts.builder(" has changed the weather to sun.").color(TextColors.GRAY).build();
