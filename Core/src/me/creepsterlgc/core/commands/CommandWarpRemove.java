...
public class CommandWarpRemove {
	...
	public CommandWarpRemove(CommandSource sender, String[] args) {
		...
		if(!PERMISSIONS.has(sender, "core.warp.remove")) { sender.sendMessage(Texts.builder("You do not have permissions!").color(TextColors.RED).build()); return; }
		...
		if(!warp.getOwner().equalsIgnoreCase(sender.getName()) && !PERMISSIONS.has(sender, "core.warp.remove-others")) {
