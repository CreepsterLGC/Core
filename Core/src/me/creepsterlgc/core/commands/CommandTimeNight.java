...
public class CommandTimeNight {
	...
	public CommandTimeNight(CommandSource sender, String[] args, Game game) {
		...
		if(!PermissionsUtils.has(sender, "core.time.night")) { sender.sendMessage(Texts.builder("You do not have permissions!").color(TextColors.RED).build()); return; }
		...
		if(args.length == 2) {
			...
			CoreServer.broadcast(Texts.of(TextColors.YELLOW, sender.getName(), TextColors.GRAY, " has changed the time to night on ", TextColors.YELLOW, world.getName()));
		...
		game.getServer().getBroadcastSink().sendMessage(Texts.of(TextColors.YELLOW, sender.getName(), TextColors.GRAY, " has changed the time to night."));
